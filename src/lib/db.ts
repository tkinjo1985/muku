...
export async function listTasks(): Promise<Task[]> {
  ...
  const rows = await db.select<TaskRow[]>(
    'SELECT id, title, priority, status, category, due, due_at, last_notified_at, created_at, updated_at FROM tasks ORDER BY created_at DESC',
  );
...
export async function addTask(task: {
  ...
  const db = await getDb();
  await db.execute(
    'INSERT INTO tasks (id, title, priority, category, due, due_at) VALUES (?, ?, ?, ?, ?, ?)',
    [
      task.id,
      task.title,
...
export async function updateTask(
  ...
  if (fields.due_at !== undefined) {
    sets.push('due_at = ?');
    args.push(fields.due_at);
  }
  if (sets.length === 0) return;
  sets.push("updated_at = datetime('now', 'localtime')");
  args.push(id);
  const db = await getDb();
  await db.execute(`UPDATE tasks SET ${sets.join(', ')} WHERE id = ?`, args);
}
...
export async function addMessage(role: 'user' | 'assistant', content: string): Promise<Message> {
  const db = await getDb();
  const res = await db.execute('INSERT INTO messages (role, content) VALUES (?, ?)', [
    role,
    content,
  ]);
  return {
